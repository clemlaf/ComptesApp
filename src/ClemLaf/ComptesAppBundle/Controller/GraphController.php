...
class GraphController extends Controller
{
  public function graphAction($type, $querystr, $param, $cpsarray){
    ...
    $query=$em->createQuery('SELECT e FROM ClemLafComptesAppBundle:Comptes\Entree e LEFT JOIN e.category c '.$querystr)->setParameters($param);
	...
	$myData->addPoints(($out+$in)/100,'solde');
      ...
      break;
    case 'pie':
      $somcat=$em->createQuery('SELECT c.cNam, SUM(e.pr) as sol FROM ClemLafComptesAppBundle:Comptes\Entree e JOIN e.category c'.str_replace('ORDER',' GROUP BY c.id ORDER',$querystr))->setParameters($param)->getResult();
      foreach($somcat as $s){
	if($s['sol']<0){
